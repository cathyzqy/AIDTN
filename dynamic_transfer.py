...
def dynamic_transfer(num_workers, sequence, orchestrator, sender, receiver, sender_instance,
    ...
    for interval in intervals:
        ...
        real_time_prediction.prediction(collect_dir + folder_name + '/' + str(int(N)) + '.csv', cluster)
        ...
        N, predict_throughput = N_prediction.get_N(predict_dir + folder_name + '/' + str(int(N)) + '_real.csv', cluster)
        ...
        data_throughput = data_throughput.append([{'time': predict_time, 'predict_throughput': predict_throughput}], ignore_index=True)
    ...
    data_throughput.to_csv('result/throughput/'+str(cluster)+'/predict/predict_' + str(transfer_id) + '.csv', index=None)
