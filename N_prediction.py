...
def get_N(real_data, cluster):
    ...
    # model = pickle.load(open('model/XGBoost/prp/xg_predict_n.dat', "rb"))
    metric = pd.read_csv('data/historical_data/'+str(cluster)+'/'+str(cluster)+'_data_normalize_metrics.csv')
...
if __name__ == '__main__':
    cluster = 'mrp'
    get_N('data/real_time_data/'+str(cluster)+'/predict/50/50_815/50_real.csv',cluster)
